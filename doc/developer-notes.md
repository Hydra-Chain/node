...
Run with the -testnet option to run with "play LOCs" on the test network, if you
...
LockTrip is a multithreaded application, and deadlocks or other multithreading bugs
...
- BitcoinMiner : Generates LOCs (if wallet is enabled).
...
However, in open source software such as LockTrip, where everyone uses
...
pay attention to for reviewers of LockTrip code.
...
- For `strprintf`, `LogPrint`, `LogPrintf` formatting characters don't need size specifiers
  ...
  - *Rationale*: LockTrip uses tinyformat, which is type safe. Leave them out to avoid confusion
...
- When looking at other's pull requests, it may make sense to add the following section to your `.git/config`
  file:
        ...
        [remote "upstream-pull"]
                ...
                url = git@github.com:LockTrip/Blockchain.git
