...
*Setup instructions for a Gitian build of LT using a Debian VM or physical system.*

Gitian is the deterministic build process that is used to build the LT
...
Clone the git repositories for LT and Gitian.
...
in the LT repository under 'Fetch and create inputs' to install sources which require
...
To build LT (for Linux, OS X and Windows) just follow the steps under 'perform
Gitian builds' in [doc/release-process.md](release-process.md#perform-gitian-builds) in the LT repository.
...
and the LT git repository with the desired tag must both be available locally, and then gbuild must be
