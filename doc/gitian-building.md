...
*Setup instructions for a Gitian build of HYDRA using a Debian VM or physical system.*

Gitian is the deterministic build process that is used to build the HYDRA
...
to https://github.com/Hydra-Chain/node/releases.
...
Clone the git repositories for HYDRA and Gitian.
...
```bash
sudo cp /home/debian/HYDRA/contrib/gitian-descriptors/xenial /usr/share/debootstrap/scripts
```
...
in the HYDRA repository under 'Fetch and create inputs' to install sources which require
...
To build HYDRA (for Linux, OS X and Windows) just follow the steps under 'perform
Gitian builds' in [doc/release-process.md](release-process.md#perform-gitian-builds) in the HYDRA repository.
...
Output from `gbuild` will look something like
    ...
    Initialized empty Git repository in /home/debian/gitian-builder/inputs/HYDRA/.git/
...
and the HYDRA git repository with the desired tag must both be available locally, and then gbuild must be
...
LXC_ARCH=amd64 LXC_SUITE=trusty on-target -u root \
  ...
  $( sed -ne '/^packages:/,/[^-] .*/ {/^- .*/{s/"//g;s/- //;p}}' ../HYDRA/contrib/gitian-descriptors/*|sort|uniq )
...
BTCPATH=/some/root/path/HYDRA
SIGPATH=/some/root/path/Detached-Sigs

./bin/gbuild --url HYDRA=${BTCPATH},signature=${SIGPATH} ../HYDRA/contrib/gitian-descriptors/gitian-win-signer.yml
```
...
```bash
    gpg --detach-sign ${VERSION}-linux/${SIGNER}/hydra-linux-build.assert
    gpg --detach-sign ${VERSION}-win/${SIGNER}/hydra-win-build.assert
    gpg --detach-sign ${VERSION}-osx-unsigned/${SIGNER}/hydra-osx-build.assert
```
...
[HYDRA/Gitian-Sigs](https://github.com/Hydra-Chain/Gitian-Sigs/) repository, or if that's not possible create a pull
request.