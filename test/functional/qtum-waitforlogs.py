...
class QtumRPCWaitforlogs(BitcoinTestFramework):
    ...
    def check_topics(self, contract_addresses,block_hashes,send_result):
        ...
        ret = self.nodes[0].waitforlogs(1602,1605,filters)

        assert_equal(ret['entries'][0]['blockHash'], block_hashes[0][0])
        assert_equal(ret['entries'][0]['blockNumber'], 1602)
    ...
    def check_waitforlogs(self, contract_addresses, send_result, block_hashes):
        ...
        ret = self.nodes[0].waitforlogs(1604,1604)

        assert_equal(ret['entries'][0]['blockHash'], block_hashes[1][0])
        assert_equal(ret['entries'][0]['blockNumber'], 1604)
        ...
        assert_equal(ret['nextblock'], 1605)
