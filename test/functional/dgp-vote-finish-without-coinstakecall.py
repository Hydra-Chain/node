...
class DgpTest(BitcoinTestFramework):
    ...
    def run_test(self):
        ...
        createvote_callstring = ("70eb3901" +
                                 ...
                                 "0" * (CALLSTRING_CHAR_COUNT - len(format(13, 'x'))) + format(3, 'x'))  # Vote duration
        ...
        (block, block_sig_key) = self.create_unsigned_pos_block(self.nodes[0], staking_prevouts[1:])
        ...
        assert_equal(self.nodes[0].getblockcount(), block_count) # Expect the block to be not accepted
