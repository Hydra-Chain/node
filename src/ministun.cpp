	...
	unsigned char *data, size_t len, stun_cb_f *stun_cb, void *arg)
{
	...
	if (len < sizeof(struct stun_header)) {
		LogPrintf("Runt STUN packet (only %d, wanting at least %d)\n", (int) len, (int) sizeof(struct stun_header));
		return -1;
	}
	...
	if (stundebug)
		LogPrintf("STUN Packet, msg %s (%04x), length: %d\n", stun_msg2str(ntohs(hdr->msgtype)), ntohs(hdr->msgtype), x);
	if (x > len) {
	...
	while (len) {
		...
		if (x > len) {
			LogPrintf("Inconsistent Attribute (length %d exceeds remaining msg len %d)\n", x, (int)len);
			break;
		}
		if (stun_cb)
			stun_cb(attr, arg);
		if (stun_process_attr(&st, attr)) {
			LogPrintf("Failed to handle attribute %s (%04x)\n", stun_attr2str(ntohs(attr->attr)), ntohs(attr->attr));
			break;
		}
	...
	*data = '\0';
	...
	if (len == 0) {
		...
		case STUN_BINDREQ:
			if (stundebug)
				LogPrintf("STUN Bind Request, username: %s\n",
		...
		default:
			if (stundebug)
				LogPrintf("Dunno what to do with STUN message %04x (%s)\n",
	...
	const char *username, struct sockaddr_in *answer)
{
	...
	for (retry = 0; retry < stuncount; retry++) {
		...
		if (res < 0) {
			LogPrintf("Request send #%d failed error %d, retry\n",
				retry, res);
			continue;
		}
		if (answer == NULL)
		...
		if (res <= 0) {	/* timeout or error */
			LogPrintf("Response read timeout #%d failed error %d, retry\n",
				retry, res);
			continue;
		}
...
int GetSTUNAddress(int client_port)
{
	int sock, res;
	...
	if( sock < 0 ) {
		LogPrintf("Error creating socket\n");
		return -1;
	}
	...
	if (!res && (mapped.sin_addr.s_addr != htonl(INADDR_ANY))) {
		//printf("STUN candidate %s:%d\n",inet_ntoa(mapped.sin_addr), htons(mapped.sin_port));
		LogPrintf("STUN candidate %s:%d\n",inet_ntoa(mapped.sin_addr), htons(mapped.sin_port));
