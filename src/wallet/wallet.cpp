...
bool CWallet::AddToWallet(const CWalletTx& wtxIn, bool fFlushOnClose)
{
    ...
    if(fUpdateAddressUnspentCache)
    {
        ...
        for (unsigned int i = 0; i < wtxIn.tx->vout.size(); i++) {
            ...
            if (!(IsSpent(*locked_chain, hash, i)) && mine != ISMINE_NO &&
...
void CWallet::AvailableCoins(interfaces::Chain::Lock& locked_chain, std::vector<COutput> &vCoins, bool fOnlySafe, const CCoinControl *coinControl, const CAmount &nMinimumAmount, const CAmount &nMaximumAmount, const CAmount &nMinimumSumAmount, const uint64_t nMaximumCount, const int nMinDepth, const int nMaxDepth, const CTxDestination& signSenderAddress, const CTxDestination& senderAddress) const
{
    ...
    for (const auto& entry : mapWallet)
    {
	    ...
	    CTxDestination dst; 
	    if (ExtractDestination(pcoin->tx->vout[i].scriptPubKey, dst)) {
                if (IsValidDestination(senderAddress) && senderAddress != dst) {
                    continue;
                }
            }
...
const CScriptCache& CWallet::GetScriptCache(const COutPoint& prevout, const CScript& scriptPubKey, std::map<COutPoint, CScriptCache>* _insertScriptCache) const
{
    ...
    if(it == prevoutScriptCache.end())
    {
        ...
        if(!scriptCache.contract)
        {
            ...
            {
                scriptCache.solvable = IsSolvable(*this, scriptPubKey);
            }
        }
...
bool CWallet::CreateTransaction(interfaces::Chain::Lock& locked_chain, const std::vector<CRecipient>& vecSend, CTransactionRef& tx, CReserveKey& reservekey, CAmount& nFeeRet,
			 ...
			 const CTxDestination& senderAddress)
{
    PriceOracle oracle;
    ...
    int nBytes;
    {
        ...
        LOCK(cs_wallet);
        {
            ...
            AvailableCoins(*locked_chain, vAvailableCoins, true, &coin_control, 1, MAX_MONEY, MAX_MONEY, 0ULL, 0, 9999999, signSenderAddress, senderAddress);
...
bool CWallet::CreateCoinStakeFromMine(interfaces::Chain::Lock& locked_chain, const CKeyStore& keystore, unsigned int nBits, const CAmount& nTotalFees, uint32_t nTimeBlock, CMutableTransaction& tx, CKey& key, std::set<std::pair<const CWalletTx*,unsigned int> >& setCoins, std::vector<COutPoint>& setSelectedCoins, bool selectedOnly, COutPoint& headerPrevout)
{
    ...
    for(const std::pair<const CWalletTx*,unsigned int> &pcoin : setPrevouts)
    {
        ...
        COutPoint prevoutStake = COutPoint(pcoin.first->GetHash(), pcoin.second);
        if (CheckKernel(pindexPrev, nBits, nTimeBlock, prevoutStake, *pcoinsTip, cache))
        {
            // Found a kernel
...
bool CWallet::CreateCoinStakeFromDelegate(interfaces::Chain::Lock& locked_chain, const CKeyStore &keystore, unsigned int nBits, const CAmount& nTotalFees, uint32_t nTimeBlock, CMutableTransaction& tx, CKey& key, std::set<std::pair<const CWalletTx*,unsigned int> >& setCoins, std::vector<COutPoint>& setDelegateCoins, std::vector<unsigned char>& vchPoD, COutPoint& headerPrevout)
{
    ...
    if(!fHasMinerStakeCache && gArgs.GetBoolArg("-stakecache", DEFAULT_STAKE_CACHE)) {
        ...
        {
            boost::this_thread::interruption_point();
            CacheKernel(stakeDelegateCache, prevoutStake, pindexPrev, *pcoinsTip); //this will do a 2 disk loads per op
        }
    }
    ...
    for(const COutPoint &prevoutStake : setDelegateCoins)
    {
        ...
        if (CheckKernel(pindexPrev, nBits, nTimeBlock, prevoutStake, *pcoinsTip, cache))
        {
            ...
            CKeyID superStakerAddress(delegation.staker);
...
const CWalletTx* CWallet::GetCoinSuperStaker(const std::set<std::pair<const CWalletTx*,unsigned int> >& setCoins, const CKeyID& superStaker, COutPoint& prevout, CAmount& nValueRet)
{
    for(const std::pair<const CWalletTx*,unsigned int> &pcoin : setCoins)
    {
        CAmount nValue = pcoin.first->tx->vout[pcoin.second].nValue;
        ...
        bool OK = false;
        CKeyID keyid = ExtractPublicKeyHash(scriptPubKey, &OK);
        if(OK && keyid == superStaker)
        {
            nValueRet = nValue;
...
std::shared_ptr<CWallet> CWallet::CreateWalletFromFile(interfaces::Chain& chain, const WalletLocation& location, uint64_t wallet_creation_flags)
{
    ...
    if (gArgs.GetBoolArg("-zapwallettxes", false) || GetStartupFileVal()) {
    ...
    if (tip_height && *tip_height != rescan_height)
    {
        ...
        if ((gArgs.GetBoolArg("-zapwallettxes", false) && gArgs.GetArg("-zapwallettxes", "1") != "2") || GetStartupFileVal())
        {
            WalletBatch batch(*walletInstance->database);
...
void CWallet::GetStakerAddressBalance(interfaces::Chain::Lock &locked_chain, const CKeyID &staker, CAmount &balance, CAmount &stake, CAmount& weight) const
{
    AssertLockHeld(cs_main);
    AssertLockHeld(cs_wallet);
    ...
    for (std::map<uint256, CWalletTx>::const_iterator it = mapWallet.begin(); it != mapWallet.end(); ++it)
    {
        ...
        {
            bool OK = false;
            CKeyID keyId = ExtractPublicKeyHash(pcoin->tx->vout[i].scriptPubKey, &OK);
            if(OK && keyId == staker)
            {
                isminetype mine = IsMine(pcoin->tx->vout[i]);
...
void CWallet::updateHaveCoinSuperStaker(const std::set<std::pair<const CWalletTx *, unsigned int> > &setCoins)
{
    ...
    for (const auto& entry : mapSuperStaker) {
        if(GetCoinSuperStaker(setCoins, CKeyID(entry.second.stakerAddress), prevout, nValueRet))
        {
...
void CWallet::UpdateMinerStakeCache(bool fStakeCache, const std::vector<COutPoint> &prevouts, CBlockIndex *pindexPrev )
{
    ...
    if(fStakeCache)
    {
        ...
        {
            boost::this_thread::interruption_point();
            CacheKernel(minerStakeCache, prevoutStake, pindexPrev, *pcoinsTip);
        }
...
void CWallet::AvailableCoinsForStaking(interfaces::Chain::Lock& locked_chain, const std::vector<uint256>& maturedTx, size_t from, size_t to, const std::map<COutPoint, uint32_t>& immatureStakes, std::vector<std::pair<const CWalletTx *, unsigned int> >& vCoins, std::map<COutPoint, CScriptCache>* insertScriptCache) const
{
...
bool CWallet::SelectCoinsForStaking(interfaces::Chain::Lock &locked_chain, CAmount &nTargetValue, std::set<std::pair<const CWalletTx *, unsigned int> > &setCoinsRet, CAmount &nValueRet) const
{
    ...
    for (std::map<uint256, CWalletTx>::const_iterator it = mapWallet.begin(); it != mapWallet.end(); ++it)
    {
        ...
        const CWalletTx* pcoin = &(*it).second;
        const CAmount tx_credit_mine{pcoin->GetAvailableCredit(locked_chain, /* fUseCache */ true, ISMINE_SPENDABLE | ISMINE_NO)};
        if(tx_credit_mine == 0)
            continue;
    ...
    if(numThreads < 2)
    {
        AvailableCoinsForStaking(locked_chain, maturedTx, 0, listSize, immatureStakes, vCoins, nullptr);
    }
    else
    {
        ...
        for(int i = 0; i < numThreads; i++)
        {
            ...
            threads.create_thread([this, from, to, &maturedTx, &immatureStakes, &vCoins]{
                auto thread_locked_chain = chain().lock();
                std::vector<std::pair<const CWalletTx *, unsigned int> > tmpCoins;
                std::map<COutPoint, CScriptCache> tmpInsertScriptCache;
                AvailableCoinsForStaking(*thread_locked_chain, maturedTx, from, to, immatureStakes, tmpCoins, &tmpInsertScriptCache);
...
bool CWallet::AvailableDelegateCoinsForStaking(const std::vector<uint160>& delegations, size_t from, size_t to, int32_t height, const std::map<COutPoint, uint32_t>& immatureStakes,  const std::map<uint256, CSuperStakerInfo>& mapStakers, std::vector<std::pair<COutPoint,CAmount>>& vUnsortedDelegateCoins, std::map<uint160, CAmount> &mDelegateWeight) const
{
    for(size_t i = from; i < to; i++)
    {
        ...
        const CKeyID& keyid = CKeyID(it->first);
