...
bool CheckTransaction(const CTransaction &tx, CValidationState &state, bool fCheckDuplicateInputs) {
    ...
    if (::GetSerializeSize(tx, SER_NETWORK, PROTOCOL_VERSION | SERIALIZE_TRANSACTION_NO_WITNESS) >
        MAX_TRANSACTION_BASE_SIZE ||
        ::GetSerializeSize(tx, SER_NETWORK, PROTOCOL_VERSION | SERIALIZE_TRANSACTION_NO_WITNESS) *
        WITNESS_SCALE_FACTOR > dgpMaxBlockWeight)
        return state.DoS(100, false, REJECT_INVALID, "bad-txns-oversize");

    if (tx.HasOpCoinstakeCall() && !tx.IsCoinStake()) {
        return state.DoS(100, false, REJECT_INVALID, "bad-txns-coinstake-call-on-not-coinstake-transaction");
    }
...
bool CheckQtumTransaction(const ExtractQtumTX & extractQtumTxResult, CValidationState& state){
    ...
    for (QtumTransaction qtumTransaction : qtumTransactions) {
        if (qtumTransaction.getQtumType() == OP_CALL && qtumTransaction.receiveAddress() == LockTripEconomyContract) {
            ...
            if (hex.substr(0, 8).compare(e.getContractFunctionHex(ADD_CONTRACT_FUNC_ID)) == 0) {
                return state.DoS(100, false, REJECT_INVALID,
                                 "bad-txns-cannot-call-addcontract-of-economy-contract");
            }
        }
