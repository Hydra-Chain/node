...
bool static ProcessMessage(CNode* pfrom, const std::string& strCommand, CDataStream& vRecv, int64_t nTimeReceived, const CChainParams& chainparams, CConnman* connman, const std::atomic<bool>& interruptMsgProc, bool enable_bip61)
{
    ...
    if (strCommand == NetMsgType::VERSION) {
        ...
        if (chainActive.Tip()->nHeight >= chainparams.GetConsensus().nLydraOverspendingFixHeight && nVersion < MIN_PEER_PROTO_VERSION_AFTER_OVERSPENDFIX) {
            // disconnect from peers older than this proto version
            LogPrint(BCLog::NET, "peer=%d using obsolete version after overspend fix hardfork %i; disconnecting\n", pfrom->GetId(), nVersion);
            if (enable_bip61) {
                connman->PushMessage(pfrom, CNetMsgMaker(INIT_PROTO_VERSION).Make(NetMsgType::REJECT, strCommand, REJECT_OBSOLETE,
                        strprintf("Version must be %d or greater after overspend fix hardfork", MIN_PEER_PROTO_VERSION_AFTER_OVERSPENDFIX)));
            }
            pfrom->fDisconnect = true;
            return false;
        }

        if (chainActive.Tip()->nHeight >= chainparams.GetConsensus().nStakerOptimizationHeight && nVersion < MIN_PEER_PROTO_VERSION_AFTER_STAKER_OPTIMIZATION) {
            // disconnect from peers older than this proto version
            LogPrint(BCLog::NET, "peer=%d using obsolete version after staker optimization fix hardfork %i; disconnecting\n", pfrom->GetId(), nVersion);
            if (enable_bip61) {
                connman->PushMessage(pfrom, CNetMsgMaker(INIT_PROTO_VERSION).Make(NetMsgType::REJECT, strCommand, REJECT_OBSOLETE,
                        strprintf("Version must be %d or greater after staker optimization fix hardfork", MIN_PEER_PROTO_VERSION_AFTER_STAKER_OPTIMIZATION)));
            }
            pfrom->fDisconnect = true;
            return false;
        }
