...
if [ -d "$1" ]; then
  ...
  echo "Removes obsolete LockTrip database files" >&2
  exit 1
fi
...
case $LEVEL in
  0)
    echo "Error: no LockTrip datadir detected."
    exit 1
    ;;
  1)
    echo "Detected old LockTrip datadir."
    echo "Nothing to do."
    exit 0
    ;;
  2)
    echo "Detected LockTrip 0.7 datadir."
    ;;
  3)
    echo "Detected LockTrip pre-0.8 datadir."
    ;;
  4)
    echo "Detected LockTrip 0.8 datadir."
    ;;
esac
