...
if [ -d "$1" ]; then
  ...
  echo "Removes obsolete HYDRA database files" >&2
  exit 1
fi
...
case $LEVEL in
  0)
    echo "Error: no HYDRA datadir detected."
    exit 1
    ;;
  1)
    echo "Detected old HYDRA datadir."
    echo "Nothing to do."
    exit 0
    ;;
  2)
    echo "Detected HYDRA 0.7 datadir."
    ;;
  3)
    echo "Detected HYDRA pre-0.8 datadir."
    ;;
  4)
    echo "Detected HYDRA 0.8 datadir."
    ;;
esac
